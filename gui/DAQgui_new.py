      ...
      if line[:1] != '#':
        ...
        try:
          ...
          if address == 0:
            ...
            volth = round((2.5*(18+91))/(18*4096)*int(vh),1)
            voltl = round((2.5*(18+91))/(18*4096)*int(vl),1)
            app.setEntry("BatLow",voltl)
            app.setEntry("BatHigh",volth)
          elif address == 4:
            th = value>>16;
            ...
            temphA = round(((int(tl)-819)/2.654+25),1)
            templA = round(((int(tl)-819)/2.654+25),1)
            temphN = round(((2500/4096)*int(th)-400)/19.5,1) 
            templN = round(((2500/4096)*int(tl)-400)/19.5,1)
          elif address == 8:
            ...
            vh = value>>16
            vl = value&0xffff
            #G1 = int(100*((2.5*37.5*(int(vh)-0.5)/4096-14)))/100
            #G2 = int(100*((2.5*37.5*(int(vl)-0.5)/4096-14)))/100
            G1 = round(((2.5*37.5*(int(vh)-0.5)/4096-14)),1)
            G2 = round(((2.5*37.5*(int(vl)-0.5)/4096-14)),1)
...
row = app.getRow()
app.addLabel("Bat","Hibernation Voltage Limits (off, on) (V)")
