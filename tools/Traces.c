...
void print_du_general(uint32_t *du)
{
  ...
  printf("\t PPS-ID %d\n",du[EVT_PPS_ID]);
  printf("\t FPGA Temp %g ADC Temp %g\n",
         ((du[EVT_FPGA_TEMP]>>16)* 509.3140064)/(1<<16) - 280.23087870,
         ((du[EVT_FPGA_TEMP]&0xffff)-819)/2.654+25);
}
...
void print_du_configuration(uint32_t *du)
{
  printf("\t Trace Length = %d ns\n",4*(du[EVT_TRACELENGTH]>>16));
  for(int ich=0;ich<3;ich++){
    printf("\t Hardware Input Channel %d ",ich+1);
    if(((du[EVT_INP_SELECT]>>5*ich)&0x1) != 0) printf("Off ");
    else{
  ...
  printf("\t Total number of sample pairs: %d\n",du[EVT_TOT_SAMPLEP]);
}

void print_du_filters(uint32_t *du)
{
