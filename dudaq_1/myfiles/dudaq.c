...
int check_server_data() {
	...
	while (DU_input[0] <= 1) {
		...
		if ((recvRet = recvfrom(DU_comms, DU_input, 2, 0,
				(struct sockaddr*) &DU_address, &RD_alength)) == 2) { //read the buffer size
	...
	while (i < DU_input[0] - 1) {
		...
		case DU_BOOT:
			...
			memcpy(
					(void*) &(shm_cmd.Ubuf[(*shm_cmd.size)
							* (*shm_cmd.next_write) + 1]), (void*) msg_start,
					2 * msg_start[AMSG_OFFSET_LENGTH]);
...
int main(int argc, char **argv) {
	...
	*(shm_ev.next_read) = 0;
	*(shm_ev.next_write) = 0;
	if (ad_shm_create(&shm_ts, BUFSIZE, sizeof(TS_DATA) / sizeof(uint16_t))
			< 0) { //ad_shm_create is in shorts!
