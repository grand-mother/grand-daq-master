...
/**
 * \fn void scope_raw_write(uint32_t, uint32_t)
 * \brief
 *
 * \param reg_addr
 * \param value
...
void scope_create_memory ()
{
   ...
   printf ("Creating %d buffers of size %d bytes, total size: %d\n", BUFSIZE, evtlen,
   BUFSIZE * evtlen);
   /**
    *
    */
   ...
   *shm_ts.next_write = 0;
   ...
   /*
    * create thread trigger T2 for high processing
    */
...
void scope_create_thread_T2 ()
{
   ...
   Gp_rbuf1trig = RBE_create (evtlen, MAX_BUF_TRIG);
   Gp_rbuf2trig = RBE_create (evtlen, MAX_BUF_TRIG);

   /* Create 2 Tensorflow Lite inference structure */
   Gp_tflt1 = TFLT_create (Gp_rbuf1trig->size_buffer);
   Gp_tflt2 = TFLT_create (Gp_rbuf1trig->size_buffer);

   /* Create 2 process of evaluation */
   Gp_feev1 = FEEV_create (Gp_rbuf1trig, (void*) Gp_tflt1);
   ...
   if (length > 0 && evtbuf != NULL)
   {
	 ...
	 *ebuf++ = scope_raw_read(Reg_Data);
      ...
      *sec = (unsigned int) timegm (&tt);
      fracsec = (double) (*(uint32_t*) &evtbuf[offset + EVT_CTD])
	    / (double) (*(uint32_t*) &evtbuf[offset + EVT_CTP]);
      nanosec = (uint32_t*) &evtbuf[offset + EVT_NANOSEC];
      *nanosec = 1.E9 * fracsec;
      evtbuf[offset + EVT_TRIGGERPOS] = shadowlist[Reg_Time1_Pre >> 1]
...
void scope_event_to_shm (uint16_t evnr, uint16_t trflag, uint16_t sec, uint32_t ssec)
{
   ...
   for (i = 0; i < BUFSIZE; i++)
   {
	 ...
	 if (nanosec >> 6 == ssec)
	 {
	    ...
	    /* memcpy(to, from, size)
	     * => evtbuf is copied by S_t3buf
	     * S_t3buf is shm_ev buffer
	     * */
	    memcpy (&S_t3buf[next_write * evtlen], &evtbuf[offset],
		    evtbuf[offset + EVT_LENGTH] * sizeof(uint16_t));
...
/**
 * \fn void scope_update_event(uint16_t, uint16_t, uint16_t, uint32_t)
 * \brief Empty buffer for evaluation and read AXI for new event
 *
 * \param evnr
 * \param trflag
 * \param sec
...
/**
 * \fn void scope_event_to_evaluation(uint16_t, uint16_t, uint16_t, uint32_t)
 * \brief
 *
 * \param evnr
 * \param trflag
 ...
 * \param ssec
 *
 * ===> call it under lock mutex ring buffer <===
 *
 */
void scope_event_to_evaluation (uint16_t evnr, uint16_t trflag, uint16_t sec, uint32_t ssec)
{
   static int S_sett = 0;
   int i;
   int offset = 0;
   ...
   {
      Gp_buftrig = Gp_rbuf2trig;
   }
   else
   {
      Gp_buftrig = Gp_rbuf1trig;
   }

   if (Gp_buftrig->nb_write == MAX_BUF_TRIG)
   {
      /* Saturation */
      if (S_sett == 0)
      {
	 printf ("Buffer trigger is saturated\n");
	 S_sett = 1;
      }
   }
   else
 ...
 *
 */
void scope_evaluation_to_trigger (void)
{
   int next_write = *(shm_ev.next_write);

   while (Gp_rbuf1trig->nb_trig)
   {
      next_write = *(shm_ev.next_write);
      if (Gp_rbuf1trig->a_prob[Gp_rbuf1trig->inext_trig] > TRIG_THRESHOLD)
      {
   ...
   {
      next_write = *(shm_ev.next_write);
      if (Gp_rbuf2trig->a_prob[Gp_rbuf2trig->inext_trig] > TRIG_THRESHOLD)
      {
	 uint32_t idx_eval_buffer = Gp_rbuf2trig->inext_trig;
	 /* index (in byte ) where start buffer to evaluate */
	 uint32_t idx_eval_byte = ((uint32_t) idx_eval_buffer) * Gp_rbuf2trig->size_buffer;
	 memcpy (&(S_t3buf[next_write * evtlen]),
		 &(Gp_rbuf2trig->a_buffers[idx_eval_byte]),
		 Gp_rbuf2trig->size_buffer);
	 next_write++;
	 if (next_write >= MAXT3)
	    next_write = 0;
	 *(shm_ev.next_write) = next_write;
