...
void scope_start_run ()
{
  if (axi_ptr == NULL)
    return;
  scope_flush ();
  scope_set_parameters (
  Reg_Dig_Control,
			shadowlist[Reg_Dig_Control >> 2] | (CTRL_PPS_EN | CTRL_SEND_EN), 1);
}
...
void scope_create_memory ()
{
  ...
  for (int ich = 0; ich < 4; ich++)
  {
    ...
    {
      evtlen += (sl[(Reg_Time1_Pre >> 1) + 2 * ich] + sl[(Reg_Time1_Post >> 1) + 2 * ich]
	  + sl[Reg_Time_Common >> 1]);
    }
  }
  if (S_rbuf_evt != NULL)
  {
  ...
  S_rbuf_evt = (uint16_t*) malloc (BUFSIZE * evtlen * sizeof(uint16_t));
  ptr_evt = 0;
  S_t3buf = (uint16_t*) shm_ev.Ubuf;//T3 requests will be in here
  *shm_ts.next_read = 0;
  *shm_ts.next_write = 0;
}
...
int scope_fake_gps()
{
  ...
  memcpy(&ppsbuf[offset+PPS_CTD],&S_rbuf_evt[next_write*evtlen+EVT_CTD],4);
...
int scope_fake_event(int32_t ioff)
{
  ...
  S_rbuf_evt[offset+EVT_LENGTH] = evtlen;
  S_rbuf_evt[offset+EVT_ID] = MAGIC_EVT;
  S_rbuf_evt[offset+EVT_HARDWARE] = station_id;
  S_rbuf_evt[offset+EVT_HDRLEN] = HEADER_EVT;
  memcpy(&S_rbuf_evt[offset+EVT_HDRLEN],&tvFake.tv_sec,4);
  memcpy(&S_rbuf_evt[offset+EVT_HDRLEN],&nanoseconds,4);
  S_rbuf_evt[offset+EVT_TRIGGERPOS] = sl[Reg_Time1_Pre>>1]+sl[Reg_Time_Common>>1];
  S_rbuf_evt[offset+EVT_VERSION] = FORMAT_EVT;
  S_rbuf_evt[offset+EVT_MSPS] = SAMPLING_FREQ;
  S_rbuf_evt[offset+EVT_ADC_RES] = ADC_RESOLUTION;
  S_rbuf_evt[offset+EVT_INP_SELECT] = sl[Reg_Inp_Select>>1];
  S_rbuf_evt[offset+EVT_CH_ENABLE] = shadowlist[Reg_TestPulse_ChRead]&0xf;
  S_rbuf_evt[offset+EVT_TOT_SAMPLES] = evtlen-HEADER_EVT;
  if(S_rbuf_evt[offset+EVT_CH_ENABLE]&1)
    S_rbuf_evt[offset+EVT_CH1_SAMPLES] = sl[(Reg_Time1_Pre>>1)]+sl[(Reg_Time1_Post>>1)]+sl[Reg_Time_Common>>1];
  else
    S_rbuf_evt[offset+EVT_CH1_SAMPLES] = 0;
  if(S_rbuf_evt[offset+EVT_CH_ENABLE]&2)
    S_rbuf_evt[offset+EVT_CH2_SAMPLES] = sl[(Reg_Time2_Pre>>1)]+sl[(Reg_Time2_Post>>1)]+sl[Reg_Time_Common>>1];
  else
    S_rbuf_evt[offset+EVT_CH2_SAMPLES] = 0;
  if(S_rbuf_evt[offset+EVT_CH_ENABLE]&4)
    S_rbuf_evt[offset+EVT_CH3_SAMPLES] = sl[(Reg_Time3_Pre>>1)]+sl[(Reg_Time3_Post>>1)]+sl[Reg_Time_Common>>1];
  else
    S_rbuf_evt[offset+EVT_CH3_SAMPLES] = 0;
  if(S_rbuf_evt[offset+EVT_CH_ENABLE]&8)
    S_rbuf_evt[offset+EVT_CH4_SAMPLES] = sl[(Reg_Time4_Pre>>1)]+sl[(Reg_Time4_Post>>1)]+sl[Reg_Time_Common>>1];
  else
    S_rbuf_evt[offset+EVT_CH4_SAMPLES] = 0;
  S_rbuf_evt[offset+EVT_TRIG_PAT] = 0;
  S_rbuf_evt[offset+EVT_TRIG_RATE] = ppsbuf[prevgps*WCNT_PPS+PPS_TRIG_RATE]; // from PPS!
  tbuf =nanoseconds/(1000/SAMPLING_FREQ);
  memcpy(&S_rbuf_evt[offset+EVT_CTD],&tbuf,4);
  memcpy(&S_rbuf_evt[offset+EVT_CTP],&ppsbuf[prevgps*WCNT_PPS+PPS_CTP],2*(WCNT_PPS-PPS_CTP));
  memcpy(&S_rbuf_evt[offset+EVT_CTRL],&sl[Reg_Dig_Control>>1],8*sizeof(uint16_t));
  memcpy(&S_rbuf_evt[offset+EVT_WINDOWS],&sl[Reg_Time1_Pre>>1],8*sizeof(uint16_t));
  memcpy(&S_rbuf_evt[offset+EVT_CHANNEL],&sl[Reg_ADC1_Gain>>1],4*6*sizeof(uint16_t));
  memcpy(&S_rbuf_evt[offset+EVT_TRIGGER],&sl[Reg_Trig1_ThresA>>1],4*6*sizeof(uint16_t));
  memcpy(&S_rbuf_evt[offset+EVT_FILTER1],&sl[Reg_FltA1_A1>>1],4*4*8*sizeof(uint16_t));
  //followed by ADC....
  if(Send_10 ==0){
    Send_10 = 1;
  }else{
...
/**
 * \fn int scope_read_event(int32_t)
 * \brief AXI to S_rbuf_evt
 *
 * \pre
 * \post
 * \param ioff
...
int scope_read_event (int32_t ioff)
{
  ...
  if ((Is_Data & (GENSTAT_EVTFIFO)) == 0)
  {
    ...
    return (0);
  if ((tbuf >> 16) == 0xADC0 || S_rbuf_evt == NULL)
    length = (tbuf & 0xffff);
  else
    length = -1;
  if (length > 0 && S_rbuf_evt != NULL)
  {
    ebuf = (uint32_t*) &S_rbuf_evt[offset];
    *ebuf++ = tbuf;
    ...
    Is_Data = scope_raw_read(Reg_GenStatus);
    S_rbuf_evt[offset + EVT_HDRLEN] = HEADER_EVT;
    sec = (uint32_t*) &S_rbuf_evt[offset + EVT_SECOND];
    tt.tm_sec = (S_rbuf_evt[offset + EVT_STATSEC] & 0xff) - S_rbuf_evt[offset + EVT_LEAP];// Convert GPS in a number of seconds
    tt.tm_min = (S_rbuf_evt[offset + EVT_MINHOUR] >> 8) & 0xff;
    tt.tm_hour = (S_rbuf_evt[offset + EVT_MINHOUR]) & 0xff;
    tt.tm_mday = (S_rbuf_evt[offset + EVT_DAYMONTH] >> 8) & 0xff;
    tt.tm_mon = (S_rbuf_evt[offset + EVT_DAYMONTH] & 0xff) - 1;
    tt.tm_year = S_rbuf_evt[offset + EVT_YEAR] - 1900;
    *sec = (unsigned int) timegm (&tt);
    fracsec = (double) (*(uint32_t*) &S_rbuf_evt[offset + EVT_CTD])
	/ (double) (*(uint32_t*) &S_rbuf_evt[offset + EVT_CTP]);
    nanosec = (uint32_t*) &S_rbuf_evt[offset + EVT_NANOSEC];
    *nanosec = 1.E9 * fracsec;
    S_rbuf_evt[offset + EVT_TRIGGERPOS] = shadowlist[Reg_Time1_Pre >> 1]
	+ shadowlist[Reg_Time_Common >> 1];
    S_rbuf_evt[offset + EVT_ID] = evtnr++;
    S_rbuf_evt[offset + EVT_HARDWARE] = station_id;
    S_rbuf_evt[offset + EVT_ATM_TEMP] = shm_mon.Ubuf[MON_TEMP];
    S_rbuf_evt[offset + EVT_ATM_PRES] = shm_mon.Ubuf[MON_PRESSURE];
    S_rbuf_evt[offset + EVT_ATM_HUM] = shm_mon.Ubuf[MON_HUMIDITY];
    S_rbuf_evt[offset + EVT_ACCEL_X] = shm_mon.Ubuf[MON_AccX];
    S_rbuf_evt[offset + EVT_ACCEL_Y] = shm_mon.Ubuf[MON_AccY];
    S_rbuf_evt[offset + EVT_ACCEL_Z] = shm_mon.Ubuf[MON_AccZ];
    S_rbuf_evt[offset + EVT_BATTERY] = shm_mon.Ubuf[MON_BATTERY];
    timestampbuf[next_write].ts_seconds = *sec;
    timestampbuf[next_write].ts_nanoseconds = *nanosec;
    timestampbuf[next_write].event_nr = S_rbuf_evt[offset + EVT_ID];
    timestampbuf[next_write].trigmask = S_rbuf_evt[offset + EVT_TRIG_PAT];
    next_write += ioff;
    if (next_write >= BUFSIZE)
      next_write = 0;
    *shm_ts.next_write = next_write;
    ptr_evt += ioff;
    if (ptr_evt >= BUFSIZE)
      ptr_evt = 0;// remember: circular buffer
...
/**
 * \fn void scope_event_to_shm(uint16_t, uint16_t, uint16_t, uint32_t)
 * \brief S_rbuf_evt to shm_ev
 *
 * \pre
 * \post
 * \param evnr
...
void scope_event_to_shm (uint16_t evnr, uint16_t trflag, uint16_t sec, uint32_t ssec)
{
  ...
  {
    nanosec = *(uint32_t*) &S_rbuf_evt[offset + EVT_NANOSEC];
    if (sec == (S_rbuf_evt[offset + EVT_SECOND] & 0xff))
    {
      if (nanosec >> 6 == ssec)
      {
	S_rbuf_evt[offset + EVT_ID] = evnr;
	S_rbuf_evt[offset + EVT_T3FLAG] = trflag;
	memcpy (&S_t3buf[next_write * evtlen], &S_rbuf_evt[offset],
		S_rbuf_evt[offset + EVT_LENGTH] * sizeof(uint16_t));
