...
int scope_open() {
  ...
  return(1);
	...
	axi_ptr = mmap(NULL, page_size, PROT_READ | PROT_WRITE, MAP_SHARED, dev,
			page_addr);
	if ((long) axi_ptr == -1) {
...
void scope_start_run() {
	if (axi_ptr == NULL)
		return;
	scope_flush();
	scope_set_parameters(Reg_Dig_Control,
...
void scope_stop_run() {
	...
	if (axi_ptr == NULL)
		return;
	scope_set_parameters(Reg_Dig_Control,
			shadowlist[Reg_Dig_Control >> 2] & (~CTRL_PPS_EN & ~CTRL_SEND_EN),
			1);
	scope_flush();
}
...
void scope_create_memory() {
	...
	for (int ich = 0; ich < 4; ich++) {
		if (shadowlist[Reg_TestPulse_ChRead >> 2] & (1 << ich)) {
			evtlen += (sl[(Reg_Time1_Pre >> 1) + 2 * ich]
					+ sl[(Reg_Time1_Post >> 1) + 2 * ich]
...
int scope_read_event(int32_t ioff) {
	...
	if (length > 0 && evtbuf != NULL) {
		...
		*ebuf++ = tbuf;
		...
		tt.tm_sec = (evtbuf[offset + EVT_STATSEC] & 0xff)
				- evtbuf[offset + EVT_LEAP]; // Convert GPS in a number of seconds
	...
	for (i = 0; i < BUFSIZE; i++) {
		...
		if (sec == (evtbuf[offset + EVT_SECOND] & 0xff)) {
			if (nanosec >> 6 == ssec) {
				...
				memcpy(&t3buf[next_write * evtlen], &evtbuf[offset],
