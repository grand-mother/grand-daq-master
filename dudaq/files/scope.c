...
unsigned int dma_wr_bit(unsigned int* dma_virtual_address, int offset, uint8_t bitNo, uint8_t bitVal) {
...
unsigned int DUreg_rd_excerpt(uint32_t reg_addr, uint8_t bitNoStart, uint8_t bitNoStop) {
  uint32_t regValue = DUreg_read(reg_addr);
  if (bitNoStart > bitNoStop) {printf("ERROR: bitNoStart sjould be smaller than bitNoStop"); exit(0);}
  unsigned int mask = 0;
...
void dma_initiate()
{
  if ((ddrOffset + eventLength + ppsLength) > (DDR_MAP_SIZE)) ddrOffset = 0;
  //ddrOffset = 0;
  /* ========================================================================================== Write SG descriptors */

  if (evtReady == 1) {  //-- CASE 1: event only DMA transfer
  ...
  else if (ppsReady == 1) { //-- CASE 3: PPS only DMA transfer
...
int dma_completion()
{
  ...
  else {                  //-- CASE 3: PPS only DMA transfer
...
int hardware_id()
{
  ...
  if (size == 12) {
      ...
      for (index = (size/4)-1; index >= 0; index--) { printf("%x ", read_data[index]); }
      printf("\n\r");
  }
  else {
      printf("size != length\n\r");
...
int scope_open()
{
  ...
  if ((long)axi_ptr == -1) { perror("opening scope\n"); exit(-1); }
  /* ========================================================================================== Map FPGA RAMs */
  // ------------------------------------------------------------- RAM header
  vadd_ram1 = (uint32_t *)mmap(0, RAM_HEADER_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM1_pbase);
  if(vadd_ram1 == (uint32_t *) -1) { printf("Can't map the RAM1 memory to user space.\n"); exit(0); }
  // ------------------------------------------------------------- RAM pre CH1
  vadd_ram2 = (uint32_t *)mmap(0, RAM_DATA_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM2_pbase);
  if(vadd_ram2 == (uint32_t *) -1) { printf("Can't map the RAM2 memory to user space.\n"); exit(0); }
  // ------------------------------------------------------------- RAM pre CH2
  vadd_ram3 = (uint32_t *)mmap(0, RAM_DATA_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM3_pbase);
  if(vadd_ram3 == (uint32_t *) -1) { printf("Can't map the RAM3 memory to user space.\n"); exit(0); }
  // ------------------------------------------------------------- RAM pre CH3
  vadd_ram4 = (uint32_t *)mmap(0, RAM_DATA_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM4_pbase);
  if(vadd_ram4 == (uint32_t *) -1) { printf("Can't map the RAM4 memory to user space.\n"); exit(0); }
  // ------------------------------------------------------------- RAM post CH1
  vadd_ram5 = (uint32_t *)mmap(0, RAM_DATA_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM5_pbase);
  if(vadd_ram5 == (uint32_t *) -1) { printf("Can't map the RAM5 memory to user space.\n"); exit(0); }
  // ------------------------------------------------------------- RAM post CH2
  vadd_ram6 = (uint32_t *)mmap(0, RAM_DATA_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM6_pbase);
  if(vadd_ram6 == (uint32_t *) -1) { printf("Can't map the RAM6 memory to user space.\n"); exit(0); }
  // ------------------------------------------------------------- RAM post CH3
  vadd_ram7 = (uint32_t *)mmap(0, RAM_DATA_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM7_pbase);
  if(vadd_ram7 == (uint32_t *) -1) { printf("Can't map the RAM7 memory to user space.\n"); exit(0); }
  // ------------------------------------------------------------- RAM pps
  vadd_ram8 = (uint32_t *)mmap(0, RAM_HEADER_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, RAM8_pbase);
  if(vadd_ram8 == (uint32_t *) -1) { printf("Can't map the RAM8 memory to user space.\n"); exit(0); }
  /* ========================================================================================== Map PSDDR */
  vadd_psddr = (uint32_t *)mmap(0, DDR_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, PSDDR_pbase);
  if(vadd_psddr == (uint32_t *) -1) { printf("Can't map the PSDDR memory to user space.\n"); exit(0); }
  /* ========================================================================================== Map CDMA regs */
  vadd_cdma = (uint32_t *)mmap(0, CDMA_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, CDMA_pbase);
  if(vadd_cdma == (uint32_t *) -1) { printf("Can't map the CDMA memory to user space.\n"); exit(0); }
  /* ========================================================================================== Map SG regs */
  vadd_sg = (uint32_t *)mmap(0, SG_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, SG_pbase);
  if(vadd_sg == (uint32_t *) -1) { printf("Can't map the SG memory to user space.\n"); exit(0); }
  ...
  if (DUreg_rd_excerpt(0x0014,1,4) != 0)   ch1En = 1;   //-- channel 1 is enabled for readout
  if (DUreg_rd_excerpt(0x0014,6,9) != 0)   ch2En = 1;   //-- channel 2 is enabled for readout
  if (DUreg_rd_excerpt(0x0014,11,14) != 0) ch3En = 1;   //-- channel 3 is enabled for readout
  ramPreSize = preOverlap + Overlap;
  ramPostSize = postOverlap;
  //ramPostSize = ramPreSize + postOverlap;
  ram2Size = 4; ram3Size = 4;
  ram4Size = 4; ram5Size = 4;
  ram6Size = 4; ram7Size = 4;
  if (ch1En == 1) { ram2Size = ramPreSize; ram5Size = ramPostSize; }
  if (ch2En == 1) { ram3Size = ramPreSize; ram6Size = ramPostSize; }
  if (ch3En == 1) { ram4Size = ramPreSize; ram7Size = ramPostSize; }
...
{
  printf("scope close\n");
  if (munmap(vadd_psddr, DDR_MAP_SIZE) == -1)       { printf("Can't unmap DDR  memory from user space.\n"); exit(0); }
  if (munmap(vadd_sg, SG_MAP_SIZE) == -1)           { printf("Can't unmap SG   memory from user space.\n"); exit(0); }
  if (munmap(vadd_cdma, CDMA_MAP_SIZE) == -1)       { printf("Can't unmap CDMA memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram1, RAM_HEADER_MAP_SIZE) == -1) { printf("Can't unmap RAM1 memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram2, RAM_DATA_MAP_SIZE) == -1)   { printf("Can't unmap RAM2 memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram3, RAM_DATA_MAP_SIZE) == -1)   { printf("Can't unmap RAM3 memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram4, RAM_DATA_MAP_SIZE) == -1)   { printf("Can't unmap RAM4 memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram5, RAM_DATA_MAP_SIZE) == -1)   { printf("Can't unmap RAM5 memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram6, RAM_DATA_MAP_SIZE) == -1)   { printf("Can't unmap RAM6 memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram7, RAM_DATA_MAP_SIZE) == -1)   { printf("Can't unmap RAM7 memory from user space.\n"); exit(0); }
  if (munmap(vadd_ram8, RAM_HEADER_MAP_SIZE) == -1) { printf("Can't unmap RAM8 memory from user space.\n"); exit(0); }
  close(dev);
  dev = 0;
}
