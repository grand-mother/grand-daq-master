...
int check_server_data() {
	...
	while (DU_input[0] <= 1) {
		...
		if ((recvRet = recvfrom(DU_comms, DU_input, INTSIZE, MSG_WAITALL,
				(struct sockaddr*) &DU_address, &RD_alength)) == INTSIZE) { //read the buffer size
			...
			while (bytesRead < (INTSIZE * DU_input[0]) && ntry < 1000) {
				...
				INTSIZE * DU_input[0] - bytesRead, 0,
...
int send_t3_event(int t3_id, int index, int evttype) {
	...
	if ((evt[EVT_LENGTH] & 0xffff) != EVT_HDR_LENGTH) {
		printf("Error requesting event. Not a proper pointer %08x ",
				vadd_psddr[shm_ev.Ubuf[index]]);
		if (index > 0)
	...
	evt[EVT_TRIGGER_STAT] = ((trflag & 0xffff) << 16)
...
void du_scope_check_commands() {
	...
	while (((shm_cmd.Ubuf[(*shm_cmd.size) * (*shm_cmd.next_read)]) & 1) == 1) { // loop over the T3 input
		...
		msg_start = (uint32_t *) (&(shm_cmd.Ubuf[(*shm_cmd.size)
				* (*shm_cmd.next_read) + 1]));
		msg_len = msg_start[AMSG_OFFSET_LENGTH];
		msg_tag = msg_start[AMSG_OFFSET_TAG];
