...
int check_server_data()
{
   ...
   while (DU_input[0] <= 1)
   {
      ...
      if ((recvRet = recvfrom(DU_comms, DU_input, INTSIZE, MSG_WAITALL,
                              (struct sockaddr*) &DU_address, &RD_alength)) == INTSIZE)
      { //read the buffer size
         ...
         while (bytesRead < (INTSIZE * DU_input[0]) && ntry < 1000)
         {
            ...
            INTSIZE * DU_input[0] - bytesRead,
                               0, (struct sockaddr*) &DU_address, &RD_alength);
            if (recvRet > 0)
               ntry = 0;
            else
               ntry++;
   ...
   while (i < DU_input[0] - 2)
   {
      ...
      switch(msg_tag)
      {
         ...
         case DU_INITIALIZE:
            ...
            memcpy((void*) &(shm_cmd.Ubuf[(*shm_cmd.size) * (*shm_cmd.next_write) + 1]),
                   (void*) msg_start,
                   INTSIZE * msg_start[AMSG_OFFSET_LENGTH]);
         ...
         case DU_GET_RANDOM_EVENT: // calculate sec and subsec, move the event to the t3 buffer
            ...
            break;
         case ALIVE:
            sendto(DU_comms, ackalive, sizeof(ackalive), 0, (struct sockaddr*) &DU_address,
                   DU_alength);
            break;
         default:
...
int socket_send(char* bf, int length)
{
   ...
   while (sentBytes < length && ntry < 100)
   {
      ...
      rsend = sendto(DU_comms, &(bf[sentBytes]), bsent, 0, (struct sockaddr*) &DU_address,
                     DU_alength);
...
{
   static int S_idx_read = 0;

   char* bf = (char*) DU_output;
   DU_alength = sizeof(DU_address);
   int32_t length;
   int ievt, imsg, iten, idx;
   uint32_t* evt0;
   uint32_t* evt1;
   ...
   if (Gp_rbuftrig[S_idx_read]->nb_trig > 0)
   {
      idx = Gp_rbuftrig[S_idx_read]->inext_trig;
      if (Gp_rbuftrig[S_idx_read]->a_prob[idx] >= TRIG_THRESHOLD)
      {
         imsg = 1;
         DU_output[imsg++] = 0;
         DU_output[imsg++] = DU_T2;
         evt0 = RBE_get_address_buffer(Gp_rbuftrig[S_idx_read], idx);
         ...
         do
         {
            ...
            if (evt1[EVT_SECOND] == evt0[EVT_SECOND])
            {
               ...
               iten = 0;
               if ((evt1[EVT_TRIGGER_STAT] & 0x100) != 0)
                  iten += 4;
               if ((evt1[EVT_TRIGGER_STAT] & 0x7f) != 0)
                  iten += 2;
         ...
         while (ievt != *(shm_ev.next_write) && imsg < (MAX_T2 - 4)
                                       && evt1[EVT_SECOND] == evt0[EVT_SECOND]); //each T2 adds 4 words
...
int send_t3_event(int t3_id, int index, int evttype)
{
   ...
   if ((evt[EVT_LENGTH] & 0xffff) != EVT_HDR_LENGTH)
   {
      ...
      if (index > 0)
         printf("( %08x ", vadd_psddr[shm_ev.Ubuf[index - 1]]);
      else
         printf("( %08x ", vadd_psddr[shm_ev.Ubuf[BUFSIZE - 1]]);
      if (index < (BUFSIZE - 1))
   ...
   trflag = 0;
   if (evttype == DU_GET_MINBIAS_EVENT)
      trflag = TRIGGER_T3_MINBIAS;
   if (evttype == DU_GET_RANDOM_EVENT)
...
void du_socket_main()
{
   ...
   while (stop_process == 0)
   {
      ...
      if (tdif >= 0.2)
      { //|| prev_msg == 1 ){                          // every 0.2 seconds, this is really only needed for phase2
         prev_msg = 0;
         //printf("Check server data\n");
         while (check_server_data() > 0)
         { // check on an Adaq command
...
void scope_create_thread_T2(void)
{
   ...
   Gp_rbuftrig[0] = RBE_create(eventLength, TRIG_MAX_BUF);
   Gp_rbuftrig[1] = RBE_create(eventLength, TRIG_MAX_BUF);
