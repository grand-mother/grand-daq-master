...
/**
 ...
 EBSITE sitename --> site where data is taken
...
int ad_init_param(char *file)
{
  ...
  while (line==fgets(line,199,fp) && tot_du < MAXDU) { // loop over all lines
    ...
    if(strcmp(key,"EBSITE") == 0){
      sscanf(line,"%s %s",key,eb_site);
    }
    if(strcmp(key,"EBEXTRA") == 0){
      sscanf(line,"%s %s",key,eb_extra);
    }
